...
class DarkOakTree {
    public static function generate($x, $y, $z, Chunk $chunk) {
        $log = BlockFactory::get(BlockIds::DARK_OAK_LOG);
        $leaves = BlockFactory::get(BlockIds::DARK_OAK_LEAVES);
        ...
        for ($i = $y; $i < $y + $trunkHeight; $i++) {
            $chunk->setBlockId($x, $i, $z, BlockIds::DARK_OAK_LOG);
            ...
            for ($leafX = $x - 2; $leafX <= $x + 2; $leafX++) {
                for ($leafZ = $z - 2; $leafZ <= $z + 2; $leafZ++) {
                    ...
                    if ($distSq <= $leafRadiusSq || ($leafRadius > 0 && $distSq <= ($leafRadius + 1) * ($leafRadius + 1))) {
                        $chunk->setBlockId($leafX, $leafY, $leafZ, BlockIds::DARK_OAK_LEAVES);
